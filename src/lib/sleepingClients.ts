...
export function buildDraftMessages(ctx: DraftMessageContext): string[] {
  ...
  return [
    `${firstName}, привет! Прошло уже ${daysSince} дн. с прошлой встречи${serviceFrag}. Записать тебя на удобное время?${sig}`,
    `${firstName}, привет 🌸 Если планируешь записаться — у меня есть свободные слоты на этой неделе. Просто скинь удобный день.${sig}`,
    `Привет, ${firstName}! Просто хотела напомнить о себе — соскучилась. Если соберёшься записаться, напиши.${sig}`,
  ];
}
