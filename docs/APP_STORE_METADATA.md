...
**Support URL:**
```
https://jumakho2013-alt.github.io/masterbook-privacy/support/
```
...
```
https://jumakho2013-alt.github.io/masterbook-privacy/privacy/
```
...
## EAS submit — настройка после получения Apple Developer

В `eas.json` намеренно убраны `ascAppId` и `appleTeamId` чтобы случайный
`eas submit -p ios --profile production` не дал cryptic-ошибку с placeholder'ом.

Когда появится Apple Developer аккаунт:

1. Создай App в App Store Connect → получи **App Apple ID** (числовой, типа `1234567890`).
2. Получи **Team ID** на developer.apple.com → Membership.
3. Передавай в submit одним из способов:

   **a) Через флаги CLI (одноразовый запуск):**
   ```bash
   eas submit -p ios --profile production \
     --asc-app-id=1234567890 \
     --apple-team-id=ABCD123EFG
   ```

   **b) Через секреты EAS (рекомендуется для CI):**
   ```bash
   eas secret:create --scope project --name ASC_APP_ID --value 1234567890
   eas secret:create --scope project --name APPLE_TEAM_ID --value ABCD123EFG
   ```
   и в `eas.json` `submit.production.ios` добавить:
   ```json
   "ascAppId": "${ASC_APP_ID}",
   "appleTeamId": "${APPLE_TEAM_ID}"
   ```

Альтернатива — интерактивный режим: `eas submit -p ios` без флагов спросит
эти значения и предложит их сохранить.
